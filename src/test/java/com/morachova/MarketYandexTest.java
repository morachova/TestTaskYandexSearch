...
public class MarketYandexTest {
    ...
    public void testNokiaLumia830Search() {
        ...
        for (int x = 1; x <= 10; x++) { //здесь можно было бы сразу прочесть какая у нас максимальная страница, вынести в int и использовать как макс. Я решила, что тест проверки более чем 10 страниц в любом случае будет очень долго прогонятся. Но при необходимости можно сделать правильнее
            if (searchItem.is(visible)) { //searchItem - переменная которая повторяется в коде дважды, в случае если мы будем искать другой предмет - проще заменить в одном месте, чем искать по всему коду. В случае множества предметов, конечно, такой подход надо будет переделать. Решила здесь не усложнять, а просто выполнить условия задания.
                System.out.println("Phone is found on page " + x + "!"); //переменная x начинается с 1, а не с 0 как раз, чтобы здесь отобразить номер страницы где мы нашли предмет (ну или начинать с 0 но в стринг добавить инкремент. Это необязательная строка
                ...
                nextPage.click();
            } else { //в случае если страницы либо закончились, либо мы дошли до 10-й, поиск остановится сообщив нам, что ничего не найдено. Поскольку в задании условие "проверяет, есть ли", а не "убедится, что результат есть" я выбрала такой путь, чтобы тест не падал в случае не нахождения, а именно искал и сообщал - есть или нет.
